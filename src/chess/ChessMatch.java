...
public class ChessMatch {
    ...
    private void undoMove(Position source, Position target, Piece capturedPiece){
        ChessPiece p = (ChessPiece)board.removedPiece(target);
        p.decreaseMoveCount();
    ...
    private void validateTargetPosition(Position source, Position target){ //valida se a pessoa pode ir na posição de destino
    ...
    private boolean testCheck(Color color){
        Position kingPosition = king(color).gertChessPosition().toPosition();
        ...
        for(Piece p: list){
            ...
            for(int i=0; i<board.getRows(); i++){
                for(int j=0; i<board.getColumns(); j++){
                    if(mat[i][j]){
                        Position source = ((ChessPiece)p).gertChessPosition().toPosition();
